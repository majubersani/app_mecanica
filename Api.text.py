...
from models import Cliente_Principal,  Veiculos, Ordens_de_servicos, local_session
...
@app.route('/cadastro_de_clientes', methods=['POST'])
def cadastro_de_cliente():
    ...
    try:
        ...
        if not dados['nome'] or not dados['cpf'] or not dados['telefone'] or not dados['endereco']:
            ...
            print(dados)
            sql = select(Cliente_Principal).where(Cliente_Principal.telefone == dados['telefone'])
            telefone_existe = db_session.execute(sql).scalar()
            print(telefone_existe)

            sql = select(Cliente_Principal).where(Cliente_Principal.cpf == dados['cpf'])
            ...
            form_novo_cliente = Cliente_Principal(
                nome=dados['nome'],
                cpf=dados['cpf'],
